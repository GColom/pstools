...
static int timeprecision = 8; //8 is the RADAR5 output precision for the time variable.
static int PSprecision   = 8; //8 is the RADAR5 output precision for phase space variables.
...
bool operator==(const PSpoint& p, const PSpoint& q){return (p.t == q.t) and (p.x == q.x) and (p.y == q.y);}
//+ operator, for summing points
PSpoint operator+(const PSpoint&p, const PSpoint& q){return PSpoint(p.t + q.t, p.x + q.x, p.y + q.y);}
...
public:
	...
	trajectory(char * path)
	{
		...
		{	
			throw std::runtime_error("Bad ifstream! You possibly tried to open a non existent record.");
		}
	}
	trajectory(){}
	...
	// between any two consecutive ones corresponds to a time tau.
	{
		...
		for(auto p : periods) res.push_back(std::lower_bound(this->begin(), this->end(), p));
	...
	// between any two consecutive ones corresponds to a full revolution.
	{
		...
		while(it != this->end())
		{
			it = std::adjacent_find(it, this->end(), [&x](PSpoint cur, PSpoint next){return (cur.x < x)&&(next.x > x);});
			if(it != this->end()) 
				{
					// At this point the it marks the first of the two PSpoints where crossing occurs.
					// We must increment it so that it points the first element after the crossing.
					// This way any ranging we might do between any two elements of res will yield 
					// the portion of orbit strictly after the first and before the last iterator provided.
					++it; 
					res.push_back(it);
				}
				else break;
	...
	{	
		...
		std::transform(beg, end - 1, beg + 1, summands.begin(), [](PSpoint cur, PSpoint next){return (cur.y + next.y)*(cur.x - next.x);});
		return std::abs(0.5 * std::accumulate(summands.begin(), summands.end(), 0.0));
	}	
...
bool operator==(const trajectory& T, const trajectory& U)
	{
		...
		if (equal_counter == sizeT) {return 1;}
		else {return 0;}
	}